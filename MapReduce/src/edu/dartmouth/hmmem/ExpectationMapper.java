...
public class ExpectationMapper extends MapReduceBase implements
	...
	public void map(LongWritable key, Text value,
		...
		Map<StringPair, Double> transLogCounts =
				calculateLogTransitionCounts(forwardMatrix, backwardMatrix, logAlpha, transLogProbMap, emisLogProbMap, observationSequence, stateSet, startState);
		Map<StringPair, Double> emisLogCounts =
				calculateLogEmissionCounts(forwardMatrix, backwardMatrix, logAlpha, observationSequence, stateSet);
	 ...
	 */
	private static Map<StringPair, Double> calculateLogTransitionCounts(Map<String, Double[]> forwardMatrix, Map<String, Double[]> backwardMatrix, Double logAlpha,
		...
		for (int i = 0; i < numObs - 1; i++) {
			...
			for (String fromState : stateSet) {
				...
				for (String toState : stateSet) {
					...
					Double sumLogTransCount = StaticUtil.calcLogSumOfLogs(prevLogTransCount, logProbToStateGivenFromState);
	 ...
	 */
	private static Map<StringPair, Double> calculateLogEmissionCounts(Map<String, Double[]> forwardMatrix, Map<String, Double[]> backwardMatrix, Double logAlpha,
